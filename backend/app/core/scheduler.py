                ...
                if not last_run_at:
                    ...
                    try:
                        ...
                        if elapsed_minutes >= interval_min:
                            should_run = True
                            logger.debug(
                                f"Rule {rule.get('name')} elapsed {elapsed_minutes:.1f}min >= {interval_min}min, scheduling...")
                        else:
                            logger.debug(
                                f"Rule {rule.get('name')} elapsed {elapsed_minutes:.1f}min < {interval_min}min, skipping...")
