  ...
  return (
    <Paper elevation={1} sx={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0, borderRadius: 1.5, overflow: 'hidden' }}>
      ...
      <Box sx={{ flex: 1, overflowY: 'auto', px: 3, py: 1.5 }}>
        ...
        <Box sx={{ px: 0.5 }}>
          <FieldRow label={t('targetIndex')}>
            <FieldValue mono>{rule.target_index}</FieldValue>
          </FieldRow>
          <FieldRow label={t('intervalMin')}>
            <FieldValue>{rule.interval_min} {t('minutes')}</FieldValue>
          </FieldRow>
          <FieldRow label={t('triggerConditionLabel')}>
            <FieldValue mono>{rule.trigger_condition || '-'}</FieldValue>
          </FieldRow>
        </Box>
        <Box sx={{ px: 0.5, mt: 0.5 }}>
          <FieldLabel>{t('defineExtractionQuery')}</FieldLabel>
          <Paper
            elevation={0}
            sx={{
              mt: 1, p: 2,
              ...
              borderRadius: 1, overflow: 'auto', maxHeight: 300,
            }}
          >
            <Typography
              component="pre"
              sx={{ fontFamily: 'monospace', fontSize: '0.72rem', lineHeight: 1.7, whiteSpace: 'pre-wrap', wordBreak: 'break-word', color: 'text.primary', m: 0 }}
            >
        ...
        <Box sx={{ px: 0.5 }}>
          <Paper
            ...
            sx={{
              ...
              bgcolor: 'background.default',
              border: '1px solid', borderColor: 'divider',
              borderRadius: 1, overflow: 'auto', maxHeight: 200,
            }}
          >
            <Typography
              component="pre"
          ...
          <FieldRow label={t('receiverRoles')}>
            <Stack direction="row" flexWrap="wrap" gap={0.5}>
              {(rule.receiver?.values || []).length > 0 ? rule.receiver.values.map((code) => {
                const label = getRoleNameFn ? getRoleNameFn(code, roleNames, language) : code;
                return (
                  <Chip key={code} label={label} size="small" variant="outlined"
                ...
                <FieldRow label={t('webhookBody')}>
                  <Paper elevation={0} sx={{ p: 1.5, bgcolor: 'background.default', border: '1px solid', borderColor: 'divider', borderRadius: 1, overflow: 'auto', maxHeight: 150 }}>
                    <Typography component="pre"
                      sx={{ fontFamily: 'monospace', fontSize: '0.68rem', lineHeight: 1.6, whiteSpace: 'pre-wrap', wordBreak: 'break-word', color: 'text.primary', m: 0 }}>
                      {rule.receiver.webhook_body}
                    </Typography>
                  </Paper>
                </FieldRow>
