...
const LogDetailPanel = React.memo(({ log, onClose, t }: { log: LogEntry; onClose: () => void; t: any }) => {
  ...
  return (
    <Box id="log-detail-panel" sx={{ width: { xs: '100%', md: '45%' }, ml: 1, display: 'flex', flexDirection: 'column', border: '1px solid', borderColor: 'divider', borderRadius: 1, bgcolor: 'background.paper', mt: 1, overflow: 'hidden', opacity: search !== deferredSearch ? 0.7 : 1, transition: 'opacity 0.2s' }}>
      ...
      <TableContainer sx={{ flexGrow: 1, overflow: 'auto', bgcolor: (theme) => theme.palette.mode === 'dark' ? 'grey.900' : 'grey.50', position: 'relative' }}><Box onMouseDown={startResizing} sx={{ position: 'absolute', left: fieldWidth, top: 0, bottom: 0, width: '6px', marginLeft: '-3px', cursor: 'col-resize', zIndex: 10, transition: 'background-color 0.2s', '&:hover': { bgcolor: 'primary.main', opacity: 0.5 }, '&:active': { bgcolor: 'primary.main', opacity: 0.8, width: '2px', marginLeft: '-1px' } }} /><Table size="small" stickyHeader sx={{ tableLayout: 'fixed' }}><TableBody>{filteredD.map((i) => (<TableRow key={i.k} hover><TableCell sx={{ width: fieldWidth, fontWeight: 'bold', fontSize: '0.75rem', color: 'primary.main', fontFamily: 'monospace', verticalAlign: 'top', borderRight: '1px solid', borderColor: 'divider', py: 1, position: 'relative', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{i.k}</TableCell><TableCell sx={{ fontSize: '0.75rem', fontFamily: 'monospace', wordBreak: 'break-all', color: 'text.primary', py: 1 }}>{i.v}</TableCell></TableRow>))}{filteredD.length === 0 && (<TableRow><TableCell colSpan={2} align="center" sx={{ py: 3, color: 'text.disabled', fontStyle: 'italic' }}>{t('noResults')}</TableCell></TableRow>)}</TableBody></Table></TableContainer>
    </Box>
  );
});
...
const LogStreamingTab: React.FC = () => {
  ...
  const t = useMemo(() => (key: string, params?: Record<string, string>): string => { const ct = translations[language] || translations["ko"] || {}; let text = ct[key] || key; if (params) Object.entries(params).forEach(([pk, v]) => { text = text.replace(`{${pk}}`, v); }); return text; }, [language]);
  ...
  useEffect(() => { if (isActive) { setLogs([]); lastTimestampRef.current = null; fetchLogs(true); } }, [searchQuery, selectedIndex, fromDate, toDate, fromValue, fromUnit, toValue, toUnit, isActive]);
  ...
  const openTimeP = (type: 'quick' | 'detailed', point: 'from' | 'to', e: React.MouseEvent<HTMLDivElement>) => { if (!isPaused) return; const cv = point === 'from' ? (fromDate || undefined) : (toDate || undefined); const d = dayjs(cv).second(0).millisecond(0); setPopoverInfo({ popoverType: type, editingPoint: point, tabValue: (point === 'from' ? (fromDate ? 0 : 1) : (toDate ? 0 : (toValue !== null ? 1 : 2))), popoverVal: (point === 'from' ? fromValue || 15 : toValue || 15), popoverUnit: (point === 'from' ? fromUnit : toUnit), popoverDate: d, popoverTime: d.format("HH:mm:ss") }); setTimeAnchorEl(e.currentTarget.parentElement as HTMLDivElement); };
      ...
      <Paper elevation={1} sx={{ p: 2, flexGrow: 1, minHeight: 0, display: 'flex', flexDirection: 'column', borderRadius: 2, overflow: 'hidden' }}>
        ...
        <Stack direction="row" sx={{ flexGrow: 1, minHeight: 0, overflow: 'hidden' }}>
          <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, position: 'relative' }}>
            <Box sx={{ display: 'flex', gap: 1, px: 3, py: 1, bgcolor: 'action.selected', borderBottom: '1px solid', borderColor: 'divider', mt: 1, borderRadius: '4px 4px 0 0' }}>{visibleFields.map((f) => (<Typography key={f} variant="caption" sx={{ fontWeight: 'bold', color: 'text.secondary', flexShrink: 0, width: f === 'timestamp' ? 230 : f === '_index' ? 180 : 'auto', flexGrow: f === 'message' ? 1 : 0, minWidth: 0 }}>{f.toUpperCase()}</Typography>))}<Box sx={{ width: 40 }} /></Box>
              ...
              {filteredLogs.length === 0 ? (<Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', gap: 1, color: 'text.disabled' }}>{loading ? <CircularProgress size={24} /> : <Typography variant="body2" sx={{ fontStyle: 'italic' }}>{t('noResults')}</Typography>}</Box>) : (
                filteredLogs.map((l) => (
                  <Box key={l._id} sx={{ py: 0.5, borderBottom: '1px solid', borderColor: 'divider', display: 'flex', alignItems: 'baseline', gap: 1, width: '100%', bgcolor: selectedLog?._id === l._id ? 'action.selected' : 'transparent', '&:last-child': { borderBottom: 'none' }, '&:hover': { bgcolor: 'action.selected' } }}>
                    {visibleFields.map((f) => (<Typography key={f} variant={f === 'message' ? 'body2' : 'caption'} sx={{ fontFamily: 'monospace', fontSize: f === 'message' ? '0.85rem' : '0.7rem', color: f === 'timestamp' ? 'text.primary' : f === '_index' ? 'text.secondary' : 'text.primary', fontWeight: (f === 'timestamp' || f === '_index') ? 'bold' : 'normal', flexShrink: f === 'message' ? 1 : 0, width: f === 'timestamp' ? 230 : f === '_index' ? 180 : 'auto', flexGrow: f === 'message' ? 1 : 0, minWidth: 0, wordBreak: 'break-all', whiteSpace: f === 'timestamp' ? 'nowrap' : 'normal', lineHeight: 1.4, ...(f === 'message' && { display: '-webkit-box', WebkitLineClamp: 5, WebkitBoxOrient: 'vertical', overflow: 'hidden', textOverflow: 'ellipsis' }), ...(f === '_index' && { bgcolor: 'action.selected', px: 0.5, borderRadius: 0.5 }) }}>{renderFV(l, f)}</Typography>))}
