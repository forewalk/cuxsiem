...
### 액션(Action) 메뉴 — 탐지·알림·에이전트 제어 통합 설계
> 사이드바에 액션(Action) 최상위 메뉴를 신설하고, 하위에 알림(Notification)을 배치.
> 향후 MITRE ATT&CK 기반 탐지 룰, 에이전트 제어(웹훅 API 호출) 등을 이 메뉴 아래로 통합 예정.
>
...
- [ ] MITRE ATT&CK 기반 탐지 룰 엔진
  ...
  - [ ] 탐지 → 알림 연계 워크플로우 설계
- [ ] 룰 엔진 아키텍처 설계 (Notification · Action · Rule 분리 구조 확정)
- [x] 웹훅 연동
...
- [ ] 라이선스 관리 UI (관리설정 > 라이선스 관리)
  - [x] 라이선스 관리 메뉴 신설 (고급설정 위, role-1 전용)
  - [x] 현재 라이선스 정보 조회 UI 플레이스홀더 (발급 대상, 만료일, 상태)
  - [x] 라이선스 파일 업로드 UI 플레이스홀더 (드래그&드롭 영역)
  - [ ] 실제 라이선스 파일 파싱 및 검증 로직 연동
