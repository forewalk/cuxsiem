...
| 2026-03-15 | 김장훈 | 추가 | 시나리오 메뉴 신설 — 프로세스 트리(피그마 기반 SVG 캔버스 레이아웃), 디텍션 룰(Elastic Security 벤치마킹) |
| 2026-03-15 | 김장훈 | 추가 | BOM 메뉴 신설 — SBOM(CycloneDX 1.6/SPDX 2.3 레이아웃), AIBOM(ml-model 확장 + 향후 계획) |
| 2026-03-15 | 김장훈 | 수정 | 사이드메뉴 초기값 모두 접힘 상태로 변경 (localStorage 제거, 로그인·새로고침·로고 클릭 시 초기화) |
| 2026-03-15 | 김장훈 | 수정 | 사이드바 액션 메뉴 신설 — 알림·알림내역·알림센터 3단 구조로 재편, ROADMAP 액션 섹션 갱신 |
| 2026-03-15 | 김장훈 | 수리 | OTP 등록 취소 시 로그인 허용 버그 수정 — 취소 시 logout() 호출로 세션 차단 |
| 2026-03-15 | 김장훈 | 수정 | 고급설정 helperText 통일 (기본값·설정 범위 형식), 역할명 필드 Tooltip→helperText 전환 |
| 2026-03-15 | 김장훈 | 수정 | 패스워드 정책 로그인실패허용횟수 범위 0~10으로 변경 (0=제한없음), helperText 통일 |
| 2026-03-15 | 김장훈 | 수리 | OTP 000000 우회 코드 순서 수정 — 복호화 이전에 체크하도록 이동 (reset-password, otp/verify-login) |
| 2026-03-15 | 김장훈 | 리팩터 | OTPLoginModal 통합 — 비밀번호 초기화용 인라인 Dialog 제거, title/showBackupCode/onSubmit props 추가 |
| 2026-03-15 | 김장훈 | 수정 | OTPEnrollModal UI 개선 — 아이콘 헤더, QR 플레이스홀더, 백업코드 2열 Chip 그리드, 성공 배너 |
| 2026-03-15 | 김장훈 | 수정 | 사용자 수정 비밀번호 필드 visibility toggle 추가, placeholder i18n 처리 |
| 2026-03-13 16:30 | 박지은 | 수정 | 다크모드 패널 배경색 통일, 알림벨 읽음 처리(localStorage 기반) 및 드롭다운 그림자·폰트 개선 |
| 2026-03-13 | 김장훈 | 배포 | origin/develop push (슬라이딩 세션, 고급설정 가이드, 사이드바 로고/홈 이동) |
...
| 2026-02-25 15:25 | 박상현 | 수정 | 로그 상세 정보 패널 검색 성능 최적화 (use DeferredValue 및 컴포넌트 분리) |
...
| 2026-03-15 08:47 | 김장훈 | 배포 | origin/develop push (계정 잠금 로직, 패스워드 정책 UI 개선, 비밀번호 복잡성 규칙, 레이아웃 수정) |
| 2026-03-17 00:00 | 김장훈 | 배포 | origin/develop push (사이드바 모니터링 메뉴 BOM 아래로 순서 변경) |
