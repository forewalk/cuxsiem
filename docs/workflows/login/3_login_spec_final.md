...
| 항목 | 변경 내용 | 사유 |
|------|----------|------|
| 초기 관리자 계정 | 수동 삽입 방식 (admin 고정) | 간단하고 안전한 초기 설정 |
| 로그인 실패 제한 | 이메일 기반 5회 실패 시 10분 차단 | ID 기반 보안 강화, 향후 관리자 해제 기능 |
| 토큰 갱신 전략 | 재로그인 필수 | 초기 버전 단순화, 보안 강화 |
| 동시 로그인 세션 | 무제한 허용 (향후 글로벌 설정) | 사용자 편의성, 향후 관리자 제어 |
| 비밀번호 정책 | 영문+숫자 필수 (향후 관리자 설정) | 보안과 사용성 균형 |
| 환경변수 | JWT_SECRET_KEY, JWT_ALGORITHM, JWT_EXPIRE_MINUTES 추가 | JWT 토큰 설정 |
| OpenSearch 매핑 | cs_users, cs_sessions, cs_login_attempts 인덱스 매핑 정의 | 데이터 구조 명확화 |
| 외부 의존성 | python-jose, passlib, python-multipart 추가 | JWT 및 비밀번호 해싱 |
...
- 로그아웃 기능
- 세션 기반 인증 (JWT 토큰)
- OpenSearch 시스템 인덱스를 통한 사용자 정보 관리 (cs_users, cs_sessions, cs_login_attempts)
- 비밀번호 암호화 (bcrypt, salt rounds 12)
- 로그인 실패 처리 및 에러 메시지
- 이메일 기반 로그인 실패 횟수 제한 (5회/10분 차단)
- 비밀번호 정책 검증 (최소 8자, 영문+숫자 필수)
- 이메일 형식 검증 및 소문자 변환
...
7. OpenSearch 시스템 인덱스 (cs_users, cs_sessions, cs_login_attempts) 생성 및 CRUD
8. 이메일 기반 로그인 실패 횟수 제한 (5회 실패 시 10분 차단, 향후 관리자 해제 기능)
9. 비밀번호 정책 검증 (최소 8자, 영문 1개 이상, 숫자 1개 이상 필수, 향후 관리자 설정 기능)
10. 이메일 형식 검증 및 소문자 변환
11. 비활성/삭제 계정 로그인 차단
...
| 2026-01-30 | 2.0  | 검토 반영 및 최종 승인 | 김장훈 |
...
### 결정된 정책
1. **초기 관리자 계정:** admin 고정, 수동 삽입
2. **로그인 실패 제한:** 이메일 기반 5회/10분 (향후 관리자 설정 가능)
3. **토큰 갱신:** 재로그인 필수
4. **동시 로그인:** 무제한 허용 (향후 글로벌 설정)
5. **비밀번호 정책:** 영문+숫자 필수 (향후 관리자 설정 가능)

### 환경변수
- JWT_SECRET_KEY (필수, 최소 32자)
- JWT_ALGORITHM (기본: HS256)
- JWT_EXPIRE_MINUTES (기본: 1440)
- JWT_EXPIRE_MINUTES_REMEMBER (기본: 10080)
...
### 향후 개발 고려사항
- 비밀번호 초기화 기능
- 로그인 정책 관리자 설정 (실패 횟수, 세션 타임아웃, 비밀번호 복잡도)
- i18n 다국어 지원
- 사용자 그룹 및 권한 관리 (인덱스 접근 제어, 페이지별 권한)
- OpenSearch _id 매핑 (id -> _id로 업데이트 처리)
